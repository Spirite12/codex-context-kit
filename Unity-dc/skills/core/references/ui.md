...
- `UIManager`
  ...
  - 接入提示：当前 UI 创建工具默认将界面逻辑脚本输出到 `Assets/Game/Scripts/Script/`，但既有 Ctrl 程序集位于 `Assets/Game/Scripts/Ctrl/`；生成前核对并显式选择落点，不自动迁移目录。生成器查重路径当前漏了 `.cs`，不要对已有脚本重复执行创建。
- `RedTip`
...
- `OnShow` / `OnHide` 中的事件和按钮订阅需要对称；自动生成按钮方法目前只补 AddListener，RemoveListener 要核对补齐。
- 测试加载中关闭、关闭后立即重开、场景销毁后异步返回；UI、红点和音频的异步结果必须归属仍有效的所有者。
- 红点接入需同时核对常量、树、工厂与 UI 绑定；静态树初始化和 `CreateRedTip` 动态路径不同，不能以静态树正常推断动态创建正常。
- 生成工具可能修改脚本和预制体，预览审查不应顺带执行生成。Prefab 拼装与最终显示验收沿用根 AGENTS.md 分工。
