...
- 默认表字段：`表名.字段名.Id`
- 枚举表：`表名.枚举名.枚举值名`
- 文本表：`表名.Sign`。当前生成器同时把 `Sign` 直接写为 C# 成员名；不要照 Inspector 提示填入 `Main.Title` 这类带点标识，先使用合法 C# 标识符或在授权后修复生成器。
- 默认表当前导出端使用 `Id`，读取表达式使用配置的 `localizeKey`；两者需一致，且 `Id` 应在本地化字段之前。调整字段顺序或自定义 key 前核对生成条目，不能仅按配置面板推定支持。
...
- 核对 key、Locale、Shared Data 与语言表引用；资源表位于 `<资源类型>/Table/`，资源文件位于语言目录。生成器当前只枚举语言目录的直接文件，不承诺递归扫描。
- 默认语言以 `Localization Settings.asset` 的选择器为准，不能从 `LocaleCode.ZhCN` 推断默认语言是中文。
- 文本表生成字段当前在实例初始化时获取翻译；涉及运行时切语时验证已创建实例能否刷新。空翻译、缺失 key 和 fallback 应分别测试。
- `Localize.LoadAsset<T>` 按 `typeof(T)` 精确查表名；先核对 `AssetTableNameDic`，不要假定 `Texture2D` 等派生类型自动匹配 `Texture`。
